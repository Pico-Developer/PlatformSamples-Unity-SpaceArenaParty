...
namespace Unity.XR.PXR
{
    public class PXR_Manager : MonoBehaviour
    {
        ...
        private static bool bindVerifyServiceSuccess;

        [HideInInspector] public bool showFps;

        [HideInInspector] public bool useDefaultFps = true;

        [HideInInspector] public int customFps;

        [HideInInspector] public bool screenFade;

        [HideInInspector] public bool eyeTracking;

        [HideInInspector] public FaceTrackingMode trackingMode = FaceTrackingMode.None;

        [HideInInspector] public bool faceTracking;

        [HideInInspector] public bool lipsyncTracking;

        [HideInInspector] public bool lateLatching;

        [HideInInspector] public FoveationLevel foveationLevel = FoveationLevel.None;

        //Entitlement Check Result
        [HideInInspector] public int appCheckResult = 100;

        [HideInInspector] public bool useRecommendedAntiAliasingLevel = true;

        private bool appSpaceWarp;

        public Action<float> DisplayRefreshRateChanged;
        private Camera[] eyeCamera;
        private readonly CameraClearFlags[] eyeCameraOriginClearFlag = new CameraClearFlags[3];
        private readonly int[] eyeCameraOriginCullingMask = new int[3];
        ...
        public static PXR_Manager Instance
        {
            get
            {
                if (instance == null)
                {
                    ...
                    if (instance == null) Debug.LogError("PXRLog instance is not initialized!");
                }
        ...
        private void Awake()
        {
            ...
            //log level
            var logLevel = PXR_Plugin.System.UPxr_GetConfigInt(ConfigType.UnityLogLevel);
            Debug.Log("PXRLog XR Platform----SDK logLevel:" + logLevel);
            PLog.logLevel = (PLog.LogLevel)logLevel;
            if (!bindVerifyServiceSuccess) PXR_Plugin.PlatformSetting.UPxr_BindVerifyService(gameObject.name);
            ...
            for (var i = 0; i < cam.Length; i++)
                if (cam[i].stereoTargetEye == StereoTargetEyeMask.Both)
                    ...
                    eyeCamera[1] = cam[i];
                else if (cam[i].stereoTargetEye == StereoTargetEyeMask.Right) eyeCamera[2] = cam[i];
            ...
            var recommendedAntiAliasingLevel =
                PXR_Plugin.System.UPxr_GetConfigInt(ConfigType.AntiAliasingLevelRecommended);
            if (useRecommendedAntiAliasingLevel && QualitySettings.antiAliasing != recommendedAntiAliasingLevel)
            {
                ...
                if (displaySubsystems.Count > 0) displaySubsystems[0].SetMSAALevel(recommendedAntiAliasingLevel);
            }

            if (openMRC && MRCInitSucceed == false) UPxr_MRCPoseInitialize();
        }
...
#endif
            if (systemFps || showFps) Camera.main.transform.Find("FPS").gameObject.SetActive(true);
        ...
        private void Update()
        {
            ...
            {
                if (PXR_Plugin.System.RecenterSuccess != null) PXR_Plugin.System.RecenterSuccess();
            ...
            {
                UPxr_GetLayerImage();
                if (createMRCOverlaySucceed) MRCUpdata();
            }
        }
        ...
        private void LateUpdate()
        {
            ...
            {
                PXR_Plugin.Render.UPxr_SetAppSpacePosition(m_AppSpaceTransform.position.x,
                    m_AppSpaceTransform.position.y, m_AppSpaceTransform.position.z);
                PXR_Plugin.Render.UPxr_SetAppSpaceRotation(m_AppSpaceTransform.rotation.x,
                    m_AppSpaceTransform.rotation.y, m_AppSpaceTransform.rotation.z, m_AppSpaceTransform.rotation.w);
            }
        }
        ...
        private void OnEnable()
        {
            ...
            if (openMRC) UPxr_MRCDataBinding();
        ...
        {
            StopAllCoroutines();
            if (openMRC) UPxr_UnMRCDataBinding();
        ...
        {
            if (openMRC && MRCInitSucceed) PXR_Plugin.Render.UPxr_DestroyLayer(99999);
        }
        ...
        private void BindVerifyServiceCallback()
        {
            bindVerifyServiceSuccess = true;
        }

        private void SeeThroughStateChangedCallback(int value)
        {
            try
            {
                for (var i = 0; i < 3; i++)
            ...
            {
                Debug.Log("PXRLog SeeThroughStateChangedCallback Error" + e);
            }
        }
        ...
        private void verifyAPPCallback(string callback)
        {
            ...
            if (EntitlementCheckResultEvent != null) EntitlementCheckResultEvent(appCheckResult);
        }
        ...
        [HideInInspector] public bool openMRC = true;

        [HideInInspector] public LayerMask foregroundLayerMask = -1;

        [HideInInspector] public LayerMask backLayerMask = -1;

        private static bool MRCInitSucceed;
        ...
        private bool mrcPlay;
        private float[] cameraAttribute;
        private PxrLayerParam layerParam;
        private float yFov = 53f;

        [HideInInspector] public GameObject backCameraObj;

        [HideInInspector] public GameObject foregroundCameraObj;

        [HideInInspector] public RenderTexture mrcRenderTexture;

        [HideInInspector] public RenderTexture foregroundMrcRenderTexture;

        private readonly Color foregroundColor = new(0, 1, 0, 1);
        ...
        public void UPxr_MRCPoseInitialize()
        {
            ...
            mrcPlay = false;
            var textureWidth = (ulong)xmlCameraData.imageWidth;
            var textureHeight = (ulong)xmlCameraData.imageHeight;
        ...
        public void UPxr_CreateMRCOverlay(uint width, uint height)
        {
            ...
            layerParam.format = (ulong)RenderTextureFormat.Default;
        ...
        public void UPxr_CopyMRCTexture(Camera cam)
        {
            ...
            {
                if (cam == null || cam.tag != Camera.main.tag ||
                    cam.stereoActiveEye == Camera.MonoOrStereoscopicEye.Right) return;
                PXR_Plugin.Render.UPxr_GetLayerNextImageIndex(99999, ref imageIndex);

                for (var eyeId = 0; eyeId < 2; ++eyeId)
                {
                    var dstT = layerTexturesInfo[eyeId].swapChain[imageIndex];
                    ...
                    if (eyeId == 0)
                        ...
                        rt = foregroundMrcRenderTexture;
                    RenderTexture tempRT = null;

                    if (!(QualitySettings.activeColorSpace == ColorSpace.Gamma && rt != null &&
                          rt.format == RenderTextureFormat.ARGB32))
                    {
                        var descriptor =
                            new RenderTextureDescriptor((int)imageW, (int)imageH, RenderTextureFormat.ARGB32, 0);
                        ...
                        if (!tempRT.IsCreated()) tempRT.Create();
                        if (eyeId == 0)
                ...
                var layerSubmit = new PxrLayerQuad();
                layerSubmit.header.layerId = 99999;
                layerSubmit.header.layerFlags = (uint)PxrLayerSubmitFlagsEXT.PxrLayerFlagMRCComposition;
        ...
        {
            var cameraDataNew = new CameraData();
            var path = Application.persistentDataPath + "/mrc.xml";
            cameraAttribute = PXR_Plugin.PlatformSetting.UPxr_MRCCalibration(path);
            Debug.Log("cameraDataLength: " + cameraAttribute.Length);
            for (var i = 0; i < cameraAttribute.Length; i++) Debug.Log("cameraData: " + i + ": " + cameraAttribute[i]);
            ...
            for (var i = 0; i < 3; i++) cameraDataNew.translation[i] = cameraAttribute[3 + i];
            for (var i = 0; i < 4; i++) cameraDataNew.rotation[i] = cameraAttribute[6 + i];
        ...
        public void UPxr_CreateCamera()
        {
            if (backCameraObj == null)
            {
                ...
                var camera = backCameraObj.AddComponent<Camera>();
            ...
            if (foregroundCameraObj == null)
            {
                ...
                var camera = foregroundCameraObj.AddComponent<Camera>();
                ...
                if (foregroundMrcRenderTexture == null)
                    foregroundMrcRenderTexture =
                        new RenderTexture((int)imageW, (int)imageH, 24, RenderTextureFormat.ARGB32);
        ...
        {
            Debug.Log("translation:" + new Vector3(translation[0], translation[1], -translation[2]));
        ...
        public Vector3 UPxr_ToRotation(float[] rotation)
        {
            ...
            var vector3 = quaternion.eulerAngles;
            Debug.Log("rotation:" + vector3);
        ...
        public void Pxr_GetHeight()
        {
            ...
            Debug.Log("Pxr_GetMrcY+:" + PXR_Plugin.System.UPxr_GetMrcY());
        }
        ...
        private void MRCUpdata()
        {
            if (PXR_Plugin.System.UPxr_GetMRCEnable())
            {
                ...
                if (foregroundCameraObj != null)
                {
                    var cameraLookAt = Camera.main.transform.position - foregroundCameraObj.transform.position;
                    var distance = Vector3.Dot(cameraLookAt, foregroundCameraObj.transform.forward);
                    foregroundCameraObj.GetComponent<Camera>().farClipPlane =
                        Mathf.Max(foregroundCameraObj.GetComponent<Camera>().nearClipPlane + 0.001f, distance);
                }

                if (backCameraObj != null && foregroundCameraObj != null) UPxr_Calibration();
            }
            else
            {
                if (mrcPlay)
        ...
        {
            if (layerImageEnable && createMRCOverlaySucceed == false)
            {
                if (layerTexturesInfo == null) layerTexturesInfo = new LayerTexture[2];
                if (PXR_Plugin.Render.UPxr_GetLayerImageCount(99999, EyeType.EyeLeft, ref imageCounts) == 0 &&
                    imageCounts > 0)
                {
                    if (layerTexturesInfo[0].swapChain == null)
                        layerTexturesInfo[0].swapChain = new Texture[imageCounts];
                    for (var j = 0; j < imageCounts; j++)
                    {
                        var ptr = IntPtr.Zero;
                        PXR_Plugin.Render.UPxr_GetLayerImagePtr(99999, EyeType.EyeLeft, j, ref ptr);
                        if (ptr == IntPtr.Zero) continue;
                        Texture sc = Texture2D.CreateExternalTexture((int)imageW, (int)imageH, TextureFormat.RGBA32,
                            false, true, ptr);
                ...
                if (PXR_Plugin.Render.UPxr_GetLayerImageCount(99999, EyeType.EyeRight, ref imageCounts) == 0 &&
                    imageCounts > 0)
                {
        ...
        public void UPxr_Calibration()
        {
            if (PXR_Plugin.System.UPxr_GetMRCEnable())
            {
                ...
                pose.position.z = 0;
                PXR_Plugin.System.UPxr_GetMrcPose(ref pose);
                backCameraObj.transform.localPosition =
                    new Vector3(pose.position.x, pose.position.y + height, -pose.position.z * 1f);
                foregroundCameraObj.transform.localPosition =
                    new Vector3(pose.position.x, pose.position.y + height, -pose.position.z * 1f);
                var rototion =
